...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/23 20:00:42 by akashets          #+#    #+#             */
/*   Updated: 2023/10/23 21:18:08 by akashets         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ((_listen_fd = socket(AF_INET, SOCK_STREAM, 0) )  == -1 ) /* create a socket (IPv4, TCP, 0) */
    {
		Logger::logMsg(CONSOLE_OUTPUT, "webserv: socket error %s   Closing ....", strerror(errno));
        exit(EXIT_FAILURE);
    }
    int option_value = 1;
    setsockopt(_listen_fd, SOL_SOCKET, SO_REUSEADDR, &option_value, sizeof(int)); /* set the option the the socket can be reused after a restart for example*/
    memset(&_server_address, 0, sizeof(_server_address)); /* set the value of _server_adress to NULL */
    ...
    {
		Logger::logMsg(CONSOLE_OUTPUT, "webserv: bind error %s   Closing ....", strerror(errno));
        exit(EXIT_FAILURE);
    }
}
